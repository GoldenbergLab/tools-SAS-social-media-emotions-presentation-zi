...
p +
  ...
  labs(x = "Sentiment", y = "log(likeCount + 1)")+
  ggtitle("Twitter")+
  theme(plot.title = element_text(hjust = 0.5),legend.position = "none");p 
...
ggplot(results, aes(y=estimate, x=term, fill=term))+geom_bar(stat='identity')+ 
  ...
  ggtitle("Emotion regression coefficients on likes on Twitter posts")+
...
# ggplot(yt, aes(x = title_vader_compound, y = log(video_view_count+1))) + geom_point()
# ggplot(yt, aes(x = title_vader_compound, y = log(video_comment_count+1))) + geom_point()
# ggplot(yt, aes(x = title_vader_compound, y = log(video_like_count+1))) + geom_point()
...
p1 +
  ...
  theme_bw() +
  labs(x = "Sentiment", y = "log(like-count + 1)")+
  ggtitle("YouTube")+
...
yt_emotions = select(yt_emotions, -c(NRC_anticip,NRC_positive,NRC_negative,NRC_overall_emotions))
...
model4 = lmer(log(score+1) ~ variable-1+(1|id), data = reddit_emotion_long); summary(model4)
confint(model4)
